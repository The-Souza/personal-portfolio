...
function Code({ className, code, ...props }: CodeProps) {
  return (
    <CodeProvider value={{ code }}>
      <div
        className={cn(
          'relative flex flex-col overflow-hidden border bg-card rounded-lg',
          className,
        )}
        {...props}
      />
...
function CodeHeader({
  ...
  return (
    <div
      className={cn(
        'bg-accent shrink-0 gap-x-2 border-b border-border/75 dark:border-border/50 text-sm flex text-accent-foreground items-center px-4 w-full h-10',
        className,
      )}
      {...props}
    >
